...
export default function Header() {
  ...
  // Detect scroll for shrink + transparency
  useEffect(() => {
    const handleScroll = () => {
          ...
          {menuItems.map((item, idx) => (
            <div key={idx} className="relative group">
              ...
              {!item.dropdown && (
                <a
                  href={item.link}
                  className="relative text-lg text-gray-800 dark:text-gray-200 font-medium group-hover:text-orange-600"
                >
                  {item.name}
              ...
              {item.dropdown && openDropdown === idx && (
                <div
                  className="absolute left-0 mt-2 w-44 bg-white dark:bg-neutral-900 shadow-lg rounded-lg py-2 border border-gray-200 dark:border-gray-700 animate-fade-down"
                  onMouseEnter={() => setOpenDropdown(idx)}
                  onMouseLeave={() => setOpenDropdown(null)}
                >
